...
/** Uncomment the lines below to dump the LF defintion to screen and check correctness */

/** 
    Export (lf_string, busted.LF);
    fprintf (stdout, lf_string, "\n");
    return 0;
*/
...
function busted.getRateDistribution (model_description, key) {
  ...
  return busted.getRateInformation.omega_info % 0;
}
...
function busted.checkForPS (model_parameters) {
   return {"omega" :Eval (((model_parameters["FG"])["omegas"])[busted.rate_classes_omega-1]),
           "weight" : Eval (((model_parameters["FG"])["weights"])[busted.rate_classes_omega-1])};
           
}

//------------------------------------------------------------------------------ 
function busted.constrainTheModel (model_parameters) {
  ExecuteCommands (((model_parameters["FG"])["omegas"])[busted.rate_classes_omega-1] + ":=1");           
}
...
function busted.io.define_bsrel_models (foreground_id, background_id, frequencies) {
    ...
    for (k = 1; k <= busted.rate_classes_alpha; k +=1) {
        tag = "" + k;
        ((model_parameters["SRV"])["omegas"]) + "srv.syn_rate_`tag`";
        if (k < busted.rate_classes_alpha) {
    ...
    busted.init_values = {};
    for (k = 0; k < busted.rate_classes_omega; k+= 1) {
        busted.init_values + (k+1)/busted.rate_classes_omega;
    }
    
    ((model_parameters["FG"])["omegas"])["busted._aux.define_parameter"][""];
    ((model_parameters["BG"])["omegas"])["busted._aux.define_parameter"][""];
    
    Eval (((model_parameters["FG"])["omegas"])[busted.rate_classes_omega-1] + ":<1e26");
    Eval (((model_parameters["FG"])["omegas"])[busted.rate_classes_omega-1] + ":>1");
    Eval (((model_parameters["BG"])["omegas"])[busted.rate_classes_omega-1] + ":<1e26");

    busted.init_values = {};
    for (k = 0; k < busted.rate_classes_omega - 1; k+= 1) {
        busted.init_values + (1-1/(k+2));
    }
    ...
    busted.init_values = {};
    for (k = 0; k < busted.rate_classes_alpha; k+= 1) {
        busted.init_values + (k+1)/busted.rate_classes_alpha;
    }

    ((model_parameters["SRV"])["omegas"])["busted._aux.define_srv_rate"][""];
    Eval (((model_parameters["SRV"])["omegas"])[busted.rate_classes_alpha-1] + ":=" + busted.rate_classes_alpha);
    
    busted.init_values = {};
    for (k = 0; k < busted.rate_classes_alpha - 1; k+= 1) {
        busted.init_values + (1-1/(k+2));
    }
...
lfunction busted.json_store_lf (json, name, ll, df, aicc, time, tree_length, tree_string, defs, has_bg) {
    ...
    rate_mx = ^("busted._srv_cat_name") + ".rates";
    weight_mx = ^("busted._srv_cat_name") + ".weights";

    srv = {^"busted.rate_classes_alpha", 2};
    for (k = 0; k < Rows (srv); k+=1) {
        srv[k][0] = (^rate_mx)[k];
        srv[k][1] = (^weight_mx)[k];
    }
    
    (((json["fits"])[name])["rate distributions"])["SRV"] = srv;
}
